...
public:
    ...
    Run(double dT,
        ...
        const HeatMap* heatmap = nullptr) const
    {
        ...
        std::transform(
            ...
            [this, &dT, &geometry, &neighborhoodSearch, &heatmap](const auto& agent) {
                return _model->ComputeNewPosition(dT, agent, geometry, neighborhoodSearch, heatmap);
            });
        ...
        std::for_each(
            ...
            [this](auto tup) {
                ...
                if(update) {
                    const Point old_pos = agent.pos;
                    _model->ApplyUpdate(*update, agent);
                    const double dist = (agent.pos - old_pos).Norm();
                    agent.fatigue = std::min(1.0, agent.fatigue + dist * agent.fatigue_rate);
                }
            });
    }
