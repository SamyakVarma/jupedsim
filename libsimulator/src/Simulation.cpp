...
void Simulation::InitializeHeatMap()
{
    if (!_geometry) return;

    // Initialize HeatMap based on true geometry bounds
    const auto& [outer, holes] = _geometry->AccessibleArea();
    AABB bounds;
    bool first = true;
    auto updateBounds = [&](const std::vector<Point>& points) {
        for (const auto& p : points) {
            if (first) {
                bounds.xmin = bounds.xmax = p.x;
                bounds.ymin = bounds.ymax = p.y;
                first = false;
            } else {
    ...
    updateBounds(outer);
    for (const auto& hole : holes) updateBounds(hole);
    
    // Add a small buffer to the bounds
    bounds.xmin -= 1.0; bounds.xmax += 1.0;
    bounds.ymin -= 1.0; bounds.ymax += 1.0;

    _heatMap = std::make_unique<HeatMap>(bounds, 0.5, _ambientTemp); // Optimized resolution: 0.5m

    // Initialize walkable mask
    for (size_t y = 0; y < _heatMap->Height(); ++y) {
        for (size_t x = 0; x < _heatMap->Width(); ++x) {
            Point p{
                bounds.xmin + x * 0.5 + 0.25, // cell center
                bounds.ymin + y * 0.5 + 0.25
            };
            if (!_geometry->InsideGeometry(p)) {
                _heatMap->SetWalkable(x, y, false);
            }
        }
    }
}
