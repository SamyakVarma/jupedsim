    ...
    const HeatMap* heatmap) const
{
    ...
    const auto& model_data = std::get<CollisionFreeSpeedModelV2Data>(ped.model);
    const auto optimal_speed = OptimalSpeed(ped, spacing, model_data.timeGap);
    
    // Heat-affected fatigue calculation
    double localTemp = heatmap ? heatmap->GetTemperature(ped.pos) : 20.0;
    double ambientTemp = heatmap ? heatmap->AmbientTemperature() : 20.0;
    
    // Scale fatigue rate: fatigue increases faster as temperature rises above ambient
    // For every 10 degrees above ambient, fatigue rate increases by 50%
    double heatEffectFactor = std::max(1.0, 1.0 + (localTemp - ambientTemp) * 0.05);
    
    // Fatigue calculation: increase fatigue based on speed, time, and heat
    double fatigueIncrement = (optimal_speed * dT * 0.001) * heatEffectFactor;
    double new_fatigue = std::min(1.0, model_data.fatigue + fatigueIncrement);
    
    // Apply fatigue to speed: speed decreases as fatigue increases
    const auto actual_speed = optimal_speed * (1.0 - new_fatigue);
    
    const auto velocity = direction * actual_speed;
    return CollisionFreeSpeedModelV2Update{ped.pos + velocity * dT, direction, new_fatigue};
}
    ...
    const
{
    ...
    // Update the persistent fatigue value in the agent's model data
    auto& model_data = std::get<CollisionFreeSpeedModelV2Data>(agent.model);
    model_data.fatigue = update.fatigue;
}
