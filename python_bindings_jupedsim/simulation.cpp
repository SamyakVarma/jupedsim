...
void init_simulation(py::module_& m)
{
    py::class_<Simulation>(m, "Simulation")
        ...
        .def("set_ambient_temperature", &Simulation::SetAmbientTemperature)
        .def("get_ambient_temperature", &Simulation::AmbientTemperature)
        .def("get_heatmap", [](const Simulation& sim) -> py::dict {
            const auto* hm = sim.GetHeatMap();
            if (!hm) return py::dict();
            ...
            bounds["xmax"] = hm->Bounds().xmax;
            bounds["ymin"] = hm->Bounds().ymin;
            bounds["ymax"] = hm->Bounds().ymax;
            d["bounds"] = bounds;
